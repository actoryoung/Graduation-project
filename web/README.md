...
### 输入模态
- **文本输入**: 直接输入中文或英文文本进行情感分析（GloVe 300维）
- **音频上传**: 支持WAV、MP3、M4A格式音频文件（实时COVAREP 74维特征提取）✅
- **视频上传**: 支持MP4、AVI、MOV格式视频文件（OpenFace 710维，待实现）⚠️
...
- ✅ 中文界面，使用UTF-8编码
- ✅ 支持灵活的模态组合输入
- ✅ 实时COVAREP音频特征提取（无需预处理）
- ✅ 双模型支持：S10-CE (单模型) + S7-集成 (加权融合)
